...
export default function FieldTaskPage() {
  ...
  async function start() { setSaving(true); const {error}=await supabase.rpc('start_my_field_task',{p_run_id:taskId}); setSaving(false); if(error)return setMessage(error.message); setMessage('تم بدء المهمة.'); await load(); }
  async function complete(reject=false) { setSaving(true); const {error}=await supabase.rpc('complete_my_field_task_step',{p_run_id:taskId,p_notes:notes.trim()||null,p_quantity:quantity?Number(quantity):null,p_gps:gps.trim()||null,p_reject:reject}); setSaving(false); if(error)return setMessage(error.message); setMessage(reject?'تم رفض المهمة وإرسال القرار.':'تم إكمال الخطوة بنجاح.'); setNotes('');setQuantity('');setGps('');await load(); }
